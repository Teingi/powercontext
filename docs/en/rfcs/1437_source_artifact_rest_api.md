...
`handoff` families, and validate `content` with the corresponding domain models. Artifact Create persists a system
Source in the same transaction solely for Revision 1 lineage. That Source must never participate in generation of
another Artifact.
...
capabilities. Artifact Create must also leave a traceable direct input while preventing that provenance-only Source
from being consumed again by a model or another generation flow.
...
## Provenance for Artifact Create

Artifact Create does not accept Source references. The server persists the validated and canonicalized Artifact
`content` as a system Source with `source_type=content`, then makes it the only direct Source lineage of Revision 1.
...
`(scope_id, family, artifact_id, revision=1)`. Public Source responses do not expose those internal-purpose fields.

Artifact Create writes the system Source, journal position, Artifact Revision 1, head, and Source lineage in one
database transaction. Any failure rolls back the entire operation.
...
the next immutable Revision and returns the new `ArtifactRevision` and ETag. The operation supports neither merge
patch nor automatic merging.

## System Source invariants

The system Source generated by Artifact Create uses public `source_type=content`. Its public `content` is the
family-specific validated and canonicalized Artifact Create content. The server-reserved portion of the typed
payload contains:
...
A `lineage_only` Source can be written only to the lineage of its target Revision 1. Explicit generation, Propose,
...
This rule applies only to Artifact Create and Revision 1. Replace does not automatically create another
`lineage_only` Source; its lineage continues to follow the existing evolution rules for that family.
...
The atomic Artifact Create transaction proceeds as follows:

```text
1. Validate scope_id, family, and content
2. Generate artifact_id and source_id
3. Build the lineage_only Source with a known target
4. Insert pc_sources and allocate journal_position
5. Insert pc_artifacts Revision 1 and pc_artifact_heads
6. Insert pc_artifact_lineage_sources with ordinal = 0
...
Source Create first and then create the Artifact in another transaction.
...
| `content` | `pc_sources.payload` | `encoded` | Content Source body; a system Source stores canonical Artifact Create content. |
...
| `operation=artifact_create` | Record foundational Artifact Create input. |
...
| `target.revision=1` | Allow lineage only for target Revision 1. |
...
4. create the target-bound `lineage_only` Source, Revision 1, head, and lineage in the Artifact Create transaction;
...
- Artifact Create atomically generates a target-bound `lineage_only` Source as the sole Source lineage of Revision 1;
...
- Source-window consumers advance over filtered records and return no-op without model or Revision when all are
  filtered;
- any failure writing Source, Artifact Revision 1, head, or lineage rolls back all writes;
...
- every Artifact Create writes an additional Source and lineage row, growing the Source journal;
...
## Generate a system Source during Artifact Create

Caller-supplied lineage references cannot ensure that every Create has a real direct input and expand the surface
for invalid references. A target-bound `lineage_only` Source created in the transaction provides provenance,
atomicity, and generation isolation together.
