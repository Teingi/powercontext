...
| --- | --- | --- | --- | --- | --- |
| Create Source | `POST /v1/sources` | `create_source` | Body 必填 `scope_id`、`source_type`、`source_id`、`content`；可选 `metadata`。`source_id` 是调用方幂等身份 | `201 Created`；返回完整 `SourceRecord`，并通过 `Location` 返回其 URI | `POST /v1/sources`<br>`{"scope_id":"git:github.com/acme/payments","source_type":"content","source_id":"refund-rule-001","content":"退款必须人工复核","metadata":{"media_type":"text/plain"}}` |
| Get Source | `GET /v1/sources/{source_id}` | `get_source` | Query 必填 `scope_id`、`source_type`；二者与 path 中 `source_id` 共同确定 Source | `200 SourceRecord`；不存在或不可见返回 `404` | `GET /v1/sources/refund-rule-001?scope_id=git%3Agithub.com%2Facme%2Fpayments&source_type=content` |
| List Sources | `GET /v1/sources` | `list_sources` | Query 必填 `scope_id`、`source_type`；可选 `limit`、`cursor`、`created_after`、`created_before`。每次仅查询一个 `source_type` | `200 SourcePage{items,next_cursor}`；按稳定顺序列举，不包含排名字段 | `GET /v1/sources?scope_id=git%3Agithub.com%2Facme%2Fpayments&source_type=content&limit=50` |
| Search Sources | `GET /v1/source-search-results` | `search_sources` | Query 必填 `scope_id`、`source_type`；可选 `q`、结构化过滤参数、`limit`、`cursor`；非空 `q` 时可选 `mode`。空白 `q` 等价于省略 | `200 SourceSearchResultPage{query,mode,hits,next_cursor}`；hit 含精确 `source_ref`，并可包含 `score` 与 `snippets` | `GET /v1/source-search-results?scope_id=git%3Agithub.com%2Facme%2Fpayments&source_type=content&q=refund%20manual%20review&mode=auto&limit=20` |
...
```http
GET /v1/source-search-results?scope_id=git%3Agithub.com%2Facme%2Fpayments&source_type=content&q=refund%20manual%20review&mode=auto&limit=20
```
...
省略 `q` 时仍调用同一 Search endpoint，返回仅由结构化过滤条件决定的搜索命中集合：

```http
GET /v1/source-search-results?scope_id=git%3Agithub.com%2Facme%2Fpayments&source_type=content&created_after=2026-09-01T00%3A00%3A00Z&limit=20
```
...
| Search Artifacts | `GET /v1/artifact-search-results` | `search_artifacts` | Query 必填 `scope_id`、`family`；可选 `q`、结构化过滤参数、`limit`、`cursor`；非空 `q` 时可选 `mode`。空白 `q` 等价于省略；首期只搜索当前可见 head | `200 ArtifactSearchResultPage{query,mode,hits,next_cursor}`；hit 含精确 `artifact_ref`，并可包含 `score`、`snippets` | `GET /v1/artifact-search-results?scope_id=git%3Agithub.com%2Facme%2Fpayments&family=experience&q=refund%20manual%20review&mode=auto&limit=20` |
...
## Artifact Get exact Revision 示例
...
```http
GET /v1/artifact-search-results?scope_id=git%3Agithub.com%2Facme%2Fpayments&family=company.example.decision&q=refund%20manual%20review&mode=auto&limit=20
```
...
省略或传入空白 `q` 时，Search 仍返回 `ArtifactSearchResultPage`，但 `query=null`、`mode=null`，命中按稳定默认顺序排列且不带排名值。
...
| HTTP status | code | 语义 |
| --- | --- | --- |
| `400/422` | `invalid_request` | 缺少 `scope_id`、字段非法、Schema 不匹配、Search mode 与请求不兼容，或 cursor 与查询条件不匹配 |
...
| 新设计 | 现有接口 | 重叠程度 | 处理方式 |
| --- | --- | --- | --- |
| `POST /v1/sources` | `POST /v1/sources/content`、`capture_content_source` | 持久化语义重叠，HTTP response 不同 | 新入口与兼容 facade 委托同一个 handler；新入口返回 `201 SourceRecord`，旧接口保留 `202 CaptureContentSourceResponse` |
