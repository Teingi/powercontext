---
title: 配置 Hermes
description: 安装 Hermes MemoryProvider，并连接到运行中的 PowerContext Server。
---

# 配置 Hermes

该集成是标准 Hermes `MemoryProvider`。Hermes 继续负责对话和 Memory 生命周期，provider 把召回、采集和显式
Memory 操作发送给单独运行的 PowerContext Server。后端故障不会中断 Hermes 对话。
...
在另一个终端安装或刷新同一 revision 的 provider：
...
Setup 命令会把 provider 复制到 `$HERMES_HOME/plugins/powercontext`，但不会启动 Server，也不会在 Hermes 中选择
provider。
...
provider 默认连接 `http://127.0.0.1:8000`。它根据当前 Hermes profile 和 gateway user identifier 推导 scope；
本地 CLI 会话没有 user identifier 时，会从 `HERMES_HOME` 推导稳定值。只有需要用不同方式共享或隔离 Memory 时才
设置显式 scope。
...
只用于 loopback Server；连接远程部署前请阅读[部署 Server](deploy-server.md)。
